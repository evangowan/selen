...
# this North America range uses up about 4.2 GB of memory at maximum, so be careful
...
since no sistematic check is made by SELEN - Two-digits 
...
awk -v path="${PATH}" '{if (NR == 3) {print "PATH="path} else {print $0} }' selen.sh > selen_temp

mv -f selen_temp selen.sh
